...
export async function POST(req: NextRequest) {
  ...
  await connectDB()
  const audit = await Audit.create({ userId: session.user.id, url, status: 'pending' })
...
async function runAudit(auditId: string, url: string, userId: string, gscSiteUrl?: string, competitorUrl?: string) {
  ...
  try {
    ...
    const [technical, onPage] = await Promise.all([analyzeTechnical(url), analyzeOnPage(url)])
    let gscData = null
    if (gscSiteUrl) gscData = await fetchGSCData(userId, gscSiteUrl)
    let competitorData = null
    if (competitorUrl) competitorData = await analyzeCompetitor(competitorUrl)
