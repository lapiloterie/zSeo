      ...
      {audit.status === 'completed' && audit.scoreBreakdown && (
        <>
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                ...
                {tab.id === 'checklist' && audit.checklist && (
                  <span className="ml-1.5 bg-emerald-500/20 text-emerald-400 text-xs px-1.5 py-0.5 rounded-full">
                    {(audit.checklist as any[]).filter(c => c.passed).length}/{(audit.checklist as any[]).length}
                  </span>
                )}
              </button>
            ))}
          ...
          <div>
            ...
            {activeTab === 'competitor' && <CompetitorTab competitor={audit.competitorData as any} onPage={audit.onPageData} technical={audit.technicalData} />}
          </div>
        </>
      )}
    </div>
...
function OverviewTab({ audit }: { audit: AuditResult }) {
  ...
  const checklistPassed = (audit.checklist as any[] || []).filter(c => c.passed).length
  const checklistTotal = (audit.checklist as any[] || []).length

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="glass rounded-xl p-5 space-y-3">
        <h3 className="font-semibold">✅ Points positifs</h3>
        ...
        {audit.onPageData?.ogComplete && <Check text="Open Graph complet" />}
        {audit.technicalData?.brokenLinks.length === 0 && <Check text="Aucun lien brisé" />}
        {checklistTotal > 0 && (
          <div className="mt-3 pt-3 border-t border-border">
            <p className="text-sm font-medium">Checklist : {checklistPassed}/{checklistTotal} validés</p>
            <div className="w-full bg-secondary rounded-full h-1.5 mt-2">
...
function LoadingCard() {
  return (
    <div className="glass rounded-2xl p-10 text-center">
      ...
      <p className="text-sm text-muted-foreground mt-2">Technique · On-Page · Core Web Vitals · Search Console · Concurrent</p>
      <div className="flex justify-center flex-wrap gap-2 mt-6">
        {['HTTPS','Sitemap','PageSpeed','GSC','Schema','Images','Liens','Lisibilité'].map(s => (
